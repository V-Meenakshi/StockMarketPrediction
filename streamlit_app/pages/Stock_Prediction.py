...
# Check if the data is not None and contains valid values
if train_df is not None and (forecast >= 0).all() and (predictions >= 0).all():
    ...
    fig.update_layout(xaxis_rangeslider_visible=False, title="Stock Price Prediction Over Time")

    # Use the native streamlit theme
    st.plotly_chart(fig, use_container_width=True)

# If the data is None or contains invalid values
else:
    # Add a title to the stock prediction graph
    st.markdown("## *Stock Prediction 📉*")
